...
from helpers import read_data_csv, read_data_json, read_data_parquet
...
class ETLPipeline(ABC):
    ...
    def transform_data(self, df: DataFrame) -> DataFrame:
        ...
        # Format authors
        df = df.withColumn("authors", f.array_join(f.col("authors"), ","))\
        ...
        df = df.alias('fact').join(
            f.broadcast(df_country_code).alias("dim"),
            on=(f.col("fact.source_country") == f.col("dim.code")),
            how="left"
        ).select(
            f.col("fact.*"),
            f.coalesce(f.col("dim.name"), f.lit("Not Given")).alias("source_country_name")
        )
