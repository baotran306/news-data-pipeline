...
class NewsAPIClient(APIClient):
    ...
    CHUNK_SIZE = 100000  # Allow fetch maximum 100000 size each time
    ...
    def __call_request(self, endpoint, params, start_point, total_news):
        ...
        while current_length < total_news:
            ...
            if respone_data and "news" in respone_data and isinstance(respone_data["news"], list):
                ls_process_news = []

                for current_new in respone_data["news"]:
                    ls_process_news.append(
                        News(
    ...
    def fetch_news(
        self, filter_query, language="en", sort_by="publish-time", sort_direction="ASC", offset=0, number=10
    ):
        ...
        params = {"text": filter_query, "language": language, "sort": sort_by, "sort-direction": sort_direction}
